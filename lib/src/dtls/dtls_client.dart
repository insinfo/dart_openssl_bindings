...
class DtlsClient {
  ...
  DtlsClient(
    ...
    String? cryptoPath,
    String? sslPath,
  })  : _libCrypto = loadLibCrypto(libCrypto, path: cryptoPath),
        _libSsl = loadLibSsl(libSsl, path: sslPath) {
    _startListening();
  }
  ...
  static Future<DtlsClient> bind(
    ...
    return DtlsClient(socket, libSsl: libSsl, libCrypto: libCrypto, cryptoPath: cryptoPath, sslPath: sslPath)
...
class _DtlsClientConnection extends Stream<Datagram> with DtlsConnection {
  ...
  Future<void> _checkSslCiphers() async {
    ...
    _libCrypto.OPENSSL_sk_free(ciphersPointer.cast());
  }
