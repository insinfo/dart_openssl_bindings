...
String _drainOpenSslErrors(OpenSslFfi lib) {
  ...
  while (true) {
    ...
    msgs.add(
        p == nullptr ? 'OpenSSL error $err' : p.cast<Utf8>().toDartString());
  }
...
mixin CryptoMixin on OpenSslContext, BioMixin {
  EvpPkey _generateKeyFromName(
    ...
    try {
      ...
      final ctx = bindings.EVP_PKEY_CTX_new_from_name(
        nullptr,
        namePtr.cast(),
        nullptr,
      );
      if (ctx == nullptr) {
        throw OpenSslException(
          'EVP_PKEY_CTX_new_from_name failed for algorithm: $algorithmName',
        );
      }
      ...
      try {
        if (bindings.EVP_PKEY_keygen_init(ctx) <= 0) {
          throw OpenSslException(
            'EVP_PKEY_keygen_init failed for algorithm: $algorithmName',
          );
        }

        if (utf8Params != null && utf8Params.isNotEmpty) {
          final params = arena<OSSL_PARAM>(utf8Params.length + 1);
          var index = 0;
          for (final entry in utf8Params.entries) {
            final keyPtr = entry.key.toNativeUtf8(allocator: arena);
            final valuePtr = entry.value.toNativeUtf8(allocator: arena);

            final param = params[index];
            param.key = keyPtr.cast();
            param.data_type = 4; // OSSL_PARAM_UTF8_STRING
            param.data = valuePtr.cast();
            param.data_size = 0;
            param.return_size = 0;
            index++;
          }

          final terminator = params[utf8Params.length];
          terminator.key = nullptr;
          terminator.data_type = 0;
          terminator.data = nullptr;
          terminator.data_size = 0;
          terminator.return_size = 0;

          if (bindings.EVP_PKEY_CTX_set_params(ctx, params) <= 0) {
            throw OpenSslException(
              'EVP_PKEY_CTX_set_params failed for algorithm: $algorithmName',
            );
          }
        }
        ...
        if (bindings.EVP_PKEY_keygen(ctx, pkeyPtr) <= 0) {
          throw OpenSslException(
            'EVP_PKEY_keygen failed for algorithm: $algorithmName',
          );
        }
  ...
  EvpPkey generateRsa(int bits) {
    ...
    try {
      if (bindings.BN_set_word(bne, 65537) != 1) {
        // 0x10001
  ...
  Uint8List pbkdf2({
    ...
    try {
      ...
      if (bindings.HMAC_Init_ex(
              ctx, passPtr.cast(), password.length, messageDigest, nullptr) !=
          1) {
  ...
  EvpPkey generateEc(String curveName) {
    return _generateKeyFromName(
      'EC',
      utf8Params: {'group': curveName},
    );
  }

  /// Generates an Ed25519 key pair.
  EvpPkey generateEd25519() {
    return _generateKeyFromName('ED25519');
  }

  /// Generates an Ed448 key pair.
  EvpPkey generateEd448() {
    return _generateKeyFromName('ED448');
  }

  /// Generates an X25519 key pair.
  EvpPkey generateX25519() {
    return _generateKeyFromName('X25519');
  }

  /// Generates an X448 key pair.
  EvpPkey generateX448() {
    return _generateKeyFromName('X448');
  }

  /// Generates an ML-DSA-44 key pair (if supported by the loaded OpenSSL).
  EvpPkey generateMlDsa44() {
    return _generateKeyFromName('ML-DSA-44');
  }

  /// Generates an ML-DSA-65 key pair (if supported by the loaded OpenSSL).
  EvpPkey generateMlDsa65() {
    return _generateKeyFromName('ML-DSA-65');
  }

  /// Generates an ML-DSA-87 key pair (if supported by the loaded OpenSSL).
  EvpPkey generateMlDsa87() {
    return _generateKeyFromName('ML-DSA-87');
  }

  /// Generates a key pair by algorithm name supported by OpenSSL providers.
  ///
  /// Examples: `RSA`, `EC`, `ED25519`, `ED448`, `X25519`, `X448`, `ML-DSA-65`.
  /// For algorithms that require textual params, pass them in [utf8Params].
  EvpPkey generateKeyByName(
    String algorithmName, {
    Map<String, String>? utf8Params,
  }) {
    return _generateKeyFromName(
      algorithmName,
      utf8Params: utf8Params,
    );
  }
